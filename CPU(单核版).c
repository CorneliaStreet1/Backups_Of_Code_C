...
void StateOutput(int mdr[],int ar[],int sr[],char IR[]);//格式上没问题了
...
int main(void)
{
    int MDR[4] = {0};//寄存器编号比对应数组下标大一
    int AR[4] = {0};//寄存器编号比对应数组下标大5
    ...
    while (l)
    {
        analyseAndPerform(ir,InstantNumber,DataField,MDR,AR,SR);
        SR[0] = SR[0] + 4;//改变标志寄存器，可以放在GetInstruction内
        l = getInstruction(ir,CodeField,InstantNumber,SR);
    }
    SR[0] = SR[0] + 4;
...
{
    FILE* file = fopen("C:\\Users\\jiangyiqing\\Desktop\\dict.dic","r");//记得自行改成你的目录，交到OJ记得改成"dict.dic"
    for(int i = 0;i < 512; i ++){
        for(int j = 0 ; j < 33 ; j ++){
            char ch = fgetc(file);
            if((ch != EOF)){
